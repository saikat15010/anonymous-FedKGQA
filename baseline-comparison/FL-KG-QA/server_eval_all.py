...
and all language models (RoBERTa, BERT, DistilBERT)
"""
...
def load_trained_models(args, all_clients_data, global_nrelation, device):
    ...
    Returns:
        (client_models, client_embeddings, global_relation_embedding, kge_model_name, lm_model_name)
    """
    ...
    # Load Language model name
    lm_model_file = os.path.join(model_dir, 'lm_model.txt')
    if os.path.exists(lm_model_file):
        with open(lm_model_file, 'r') as f:
            lm_model_name = f.read().strip()
        logging.info(f"Detected Language model: {lm_model_name.upper()}")
    else:
        # Fallback: use command line argument or default to roberta
        lm_model_name = args.lm_model.lower() if hasattr(args, 'lm_model') else 'roberta'
        logging.warning(f"Language model file not found, using: {lm_model_name.upper()}")
    
    # Update args with the correct models
    args.kge_model = kge_model_name
    args.lm_model = lm_model_name
    ...
    return client_models, client_embeddings, global_relation_embedding, kge_model_name, lm_model_name
...
def main(args):
    ...
    client_models, client_embeddings, global_relation_embedding, kge_model_name, lm_model_name = load_trained_models(
        args, all_clients_data, global_nrelation, device
    )
    
    logging.info(f"KGE Model: {kge_model_name.upper()}")
    logging.info(f"Language Model: {lm_model_name.upper()}")
    ...
    results_file = os.path.join(args.state_dir, f'server_test_results_{kge_model_name}_{lm_model_name}.json')
...
if __name__ == '__main__':
    ...
    parser.add_argument('--lm_model', type=str, default='roberta',
                       ...
                       help='Language model (will be auto-detected from saved model)')
