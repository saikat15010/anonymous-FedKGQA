"""
Baseline 2: Adapted FL-KG-QA (Horizontal Federated)

Uses FedAvg on relation embeddings and QA model parameters (same as
FedKGQA), but restricts QA to single-hop scoring only.

Differences from FedKGQA:
  - Phase 1: Same (FedAvg on relations)
  - Phase 2: Same aggregation, but QA model only does 1-hop scoring
  - No multi-hop question decomposition
  - Topic entity → direct answer (no intermediate hops)

Reference: Gunti et al., "A Federated Learning approach for Question
and Answering on Knowledge Graphs", JISEM 2024.
"""
...
class FederatedKGQA:
    """FL-KG-QA: federated training but 1-hop QA only."""
    ...
    def __init__(self, args, all_clients_data, global_nrelation):
        ...
        logging.info(f"Baseline: FL-KG-QA (1-hop only)")
    ...
    def setup_clients(self, kg_dataloaders, train_qa_loaders, dev_qa_loaders, test_qa_loaders=None):
        ...
        for i, client_data in enumerate(self.all_clients_data):
            ...
            # KEY: Use reduced top_k=1 to simulate 1-hop scoring
    ...
    def train_phase1_kg(self):
        """Phase 1: Same as FedKGQA (FedAvg on relations)"""
        logging.info("\n" + "=" * 70)
        logging.info(f"PHASE 1: Federated KG Training (FedAvg relations)")
        logging.info("=" * 70)
    ...
    def train_phase2_qa(self):
        """Phase 2: FedAvg on QA models, but 1-hop scoring only"""
        logging.info("\n" + "=" * 70)
        logging.info(f"PHASE 2: Federated QA Training (1-hop only)")
        logging.info("=" * 70)
        ...
        for round_idx in range(self.args.qa_max_rounds):
            ...
            # FedAvg on QA models (same as FedKGQA)
    ...
    def _train_client_qa(self, client_id):
        ...
        for epoch in range(self.args.qa_local_epoch):
            for questions, answer_ids, hop_counts in self.train_qa_loaders[client_id]:
                # KEY: Force all questions to be treated as 1-hop
                hop_counts_1hop = [1] * len(questions)
