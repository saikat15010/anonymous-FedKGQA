...
class QuestionEncoder(nn.Module):
    """
    Universal Question encoder supporting multiple language models:
    ...
    def __init__(self, args, hidden_dim):
        ...
        self.args = args
        self.lm_type = args.lm_model.lower()  # 'roberta', 'bert', or 'distilbert'
        
        torch.set_num_threads(1)
        
        # Select model based on lm_type
        if self.lm_type == 'roberta':
            ...
            raise ValueError(f"Unknown language model: {self.lm_type}. Choose from: roberta, bert, distilbert")
    ...
    def forward(self, questions):
        ...
        # For RoBERTa and BERT: first token in last_hidden_state
        # For DistilBERT: first token in last_hidden_state
        cls_embeddings = outputs.last_hidden_state[:, 0, :]  # [batch_size, lm_hidden_size]
...
class ImprovedKGQAModel(nn.Module):
    ...
    1. Universal question encoder (RoBERTa/BERT/DistilBERT)
    ...
    def __init__(self, args, nentity, nrelation):
        ...
        self.lm_type = args.lm_model.lower()
        
        # Question encoder (supports RoBERTa, BERT, DistilBERT)
        self.question_encoder = QuestionEncoder(args, args.hidden_dim)
