...
class ComplExModel(nn.Module):
    ...
    ComplEx uses complex-valued embeddings and is particularly effective
    for capturing symmetric and antisymmetric relations.
    
    Score = Re(<h, r, conj(t)>) where <> is complex dot product
    """
    ...
    def forward(self, sample, relation_embedding, entity_embedding, neg=True):
        ...
        Args:
            ...
            relation_embedding: [nrelation, 2*hidden_dim] (real + imaginary parts)
    ...
    def complex_score(self, head, relation, tail):
        ...
        Score = Re(<h, r, conj(t)>)
        where h, r, t are complex embeddings
        ...
        # Second: (h * r) * conj(t)
        # Real part: re_hr * re_tail + im_hr * im_tail
        # Imaginary part: im_hr * re_tail - re_hr * im_tail
        
        # For scoring, we only need the real part
        re_score = re_hr * re_tail + im_hr * im_tail
        
        # Sum over embedding dimensions
        score = re_score.sum(dim=2)
...
def initialize_embeddings(nentity, nrelation, hidden_dim, embedding_range):
    ...
    ComplEx uses 2*hidden_dim to store real and imaginary parts
