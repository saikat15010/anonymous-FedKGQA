...
class FederatedKGQA:
    ...
    def __init__(self, args, all_clients_data, global_nrelation):
        ...
        # RotatE: relations are phases/angles, size = hidden_dim (not 2*hidden_dim like ComplEx)
        pi = 3.14159265358979323846
        global_relation_init = torch.zeros(global_nrelation, args.hidden_dim).uniform_(-pi, pi)
    ...
    def setup_clients(self, kg_dataloaders, train_qa_loaders, dev_qa_loaders, test_qa_loaders=None):
        ...
        for i, client_data in enumerate(self.all_clients_data):
            ...
            # Initialize entity embeddings (local to each client)
            # RotatE: entities are complex embeddings, size = 2*hidden_dim
    ...
    def train_phase2_qa(self):
        ...
        - KG embeddings are frozen
        - QA models are trained and aggregated via FedAvg
        """
        logging.info("\n" + "=" * 70)
        ...
        for round_idx in range(self.args.qa_max_rounds):
            ...
            # Log loss even if not evaluating
            if (round_idx + 1) % self.args.log_per_round == 0 and (round_idx + 1) % self.args.check_per_round != 0:
                logging.info(
                    f"[QA Round {round_idx + 1}/{self.args.qa_max_rounds}] "
                    f"Clients: {selected_clients}, "
                    f"Avg Loss: {avg_loss:.4f}"
