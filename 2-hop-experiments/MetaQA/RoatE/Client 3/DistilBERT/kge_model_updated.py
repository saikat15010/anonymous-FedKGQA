...
class RotatEModel(nn.Module):
    ...
    Each relation is a rotation from head to tail entity.
    
    Score = -||h ∘ r - t||  (L1 or L2 distance)
    where ∘ is complex multiplication (rotation)
    """
    def __init__(self, args):
        ...
        # Modulus for L2 normalization of relations (constrain to unit circle)
    ...
    def forward(self, sample, relation_embedding, entity_embedding, neg=True):
        ...
        Args:
            ...
            relation_embedding: [nrelation, hidden_dim] (phase/angle)
            entity_embedding: [nentity, 2*hidden_dim] (real + imaginary parts)
    ...
    def rotate_score(self, head, relation, tail):
        ...
        Args:
            ...
            relation: [batch_size, 1, hidden_dim] (phase/angle)
        ...
        # Relation is phase (angle in radians)
        # Map relation to [-pi, pi]
        phase_relation = relation / (self.embedding_range / pi)
        
        # Convert phase to rotation: r = cos(phase) + i*sin(phase)
        re_relation = torch.cos(phase_relation)
        im_relation = torch.sin(phase_relation)
        
        # Complex multiplication: h ∘ r = (re_h * re_r - im_h * im_r) + i(re_h * im_r + im_h * re_r)
        ...
        score = score.norm(dim=0)  # Compute L2 norm
        
        # Sum over embedding dimensions
        score = score.sum(dim=2)
        
        # RotatE uses distance (smaller is better), convert to similarity (larger is better)
...
def initialize_embeddings(nentity, nrelation, hidden_dim, embedding_range):
    ...
    - Relations: hidden_dim (phase/angle)
    ...
    # Initialize relation embeddings (phase) with uniform distribution in [0, 2*pi)
    pi = 3.14159265358979323846
    relation_embedding = nn.Parameter(
        torch.zeros(nrelation, hidden_dim).uniform_(-pi, pi)
    )
