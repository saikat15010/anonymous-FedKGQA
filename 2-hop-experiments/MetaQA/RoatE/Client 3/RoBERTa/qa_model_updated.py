...
class ImprovedKGQAModel(nn.Module):
    ...
    5. Updated for RotatE embeddings (relation_dim = hidden_dim, not 2*hidden_dim)
    """
    ...
    def forward(self, questions, relation_embeddings, entity_embeddings, 
        ...
        Args:
            questions: List of question strings
            relation_embeddings: [nrelation, hidden_dim] for RotatE (phase/angle)
            entity_embeddings: [nentity, hidden_dim*2] for RotatE (real + imaginary parts)
        ...
        # For RotatE, use only real part of entity embeddings for initial scoring
        # Entity embeddings: [nentity, 2*hidden_dim] (real + imaginary)
        # Relation embeddings: [nrelation, hidden_dim] (phase/angle)
        entity_emb_real = entity_embeddings[:, :self.hidden_dim]  # [nentity, hidden_dim]
        
        # Relation embeddings for RotatE are already just hidden_dim (phase), so use directly
        relation_emb = relation_embeddings  # [nrelation, hidden_dim]
        
        # Predict relation using attention
        question_emb_expanded = question_emb.unsqueeze(1)  # [batch_size, 1, hidden_dim]
        relation_emb_expanded = relation_emb.unsqueeze(0).expand(batch_size, -1, -1)  # [batch_size, nrelation, hidden_dim]
        ...
        relation_scores = torch.matmul(rel_query, relation_emb.t())  # [batch_size, nrelation]
    ...
    def predict_answers(self, questions, relation_embeddings, entity_embeddings, 
        ...
        Args:
            questions: List of question strings
            relation_embeddings: [nrelation, hidden_dim] for RotatE
            entity_embeddings: [nentity, hidden_dim*2] for RotatE
