...
def load_trained_models(args, all_clients_data, global_nrelation, device):
    ...
    if os.path.exists(lm_model_file):
        with open(lm_model_file, 'r') as f:
            args.lm_model = f.read().strip()
    else:
        # Default to roberta if not found
        args.lm_model = 'roberta'
    ...
    for client_id in range(len(all_clients_data)):
        ...
        # Initialize QA model
        qa_model = ImprovedKGQAModel(args, client_data['nentity'], global_nrelation)
