...
def init_logger(args):
    ...
    # Console handler (immediate output, no buffering)
...
def main(args):
    ...
    logging.info("=" * 70)
    logging.info(f"Federated KGQA Training - {args.kge_model.upper()} + {args.lm_model.upper()}")
    logging.info("=" * 70)
    ...
    logging.info("\n" + "=" * 70)
    logging.info("Loading MetaQA Dataset")
    logging.info("=" * 70)
    ...
    logging.info("=" * 70)
    logging.info(f"KGE Model: {args.kge_model.upper()}")
    logging.info(f"Language Model: {args.lm_model.upper()}")
    logging.info(f"Embedding dimension: {args.hidden_dim}")
    logging.info(f"Device: {args.gpu}")
    
    fkgqa = FederatedKGQA(args, all_clients_data, global_nrelation)
    ...
    logging.info(f"To evaluate on server test set, run: python server_eval_all.py --kge_model {args.kge_model} --lm_model {args.lm_model}")


if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Federated KGQA on MetaQA with Multiple KGE and Language Models')
    ...
    parser.add_argument('--name', type=str, default=None,
                       help='Experiment name (default: fkgqa_{kge_model}_{lm_model})')
    ...
    parser.add_argument('--kge_model', type=str, default='complex',
                       ...
                       help='Knowledge Graph Embedding model to use')
    parser.add_argument('--lm_model', type=str, default='roberta',
                       choices=['roberta', 'bert', 'distilbert'],
                       help='Language model for question encoding (roberta, bert, distilbert)')
    
    # Model parameters
    parser.add_argument('--hidden_dim', type=int, default=256,
                       help='Embedding dimension')
    parser.add_argument('--gamma', type=float, default=12.0,
                       help='Margin/gamma parameter for KGE models')
    parser.add_argument('--epsilon', type=float, default=2.0,
                       help='Epsilon for embedding initialization')
    parser.add_argument('--transe_norm', type=int, default=1,
                       help='Norm for TransE distance (1 or 2)')
    
    # Training parameters - Phase 1 (KG)
    parser.add_argument('--kg_max_rounds', type=int, default=10,
    ...
    parser.add_argument('--qa_max_rounds', type=int, default=5,
    ...
    parser.add_argument('--qa_batch_size', type=int, default=8,
                       help='Batch size for QA training (smaller for language models)')
    parser.add_argument('--qa_lr', type=float, default=5e-6,
                       help='Learning rate for QA training')
    parser.add_argument('--num_neg_qa', type=int, default=256,
    ...
    parser.add_argument('--check_per_round', type=int, default=2,
    ...
    if args.name is None:
        args.name = f'fkgqa_{args.kge_model}_{args.lm_model}'
