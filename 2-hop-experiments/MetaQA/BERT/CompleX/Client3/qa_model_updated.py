...
class TransformerQuestionEncoder(nn.Module):
    """
    Unified question encoder supporting BERT, DistilBERT, and RoBERTa
    
    Supported encoders:
    - bert: BERT-base (bert-base-uncased)
    - distilbert: DistilBERT-base (distilbert-base-uncased)
    - roberta: RoBERTa-base (roberta-base)
    """
    def __init__(self, args, hidden_dim, encoder_type='roberta'):
        ...
        # Select model and tokenizer based on encoder type
        if self.encoder_type == 'bert':
        ...
        elif self.encoder_type == 'distilbert':
            model_name = 'distilbert-base-uncased'
            self.tokenizer = DistilBertTokenizer.from_pretrained(model_name, use_fast=False)
            ...
            raise ValueError(f"Unsupported encoder type: {encoder_type}. "
                           f"Choose from: bert, distilbert, roberta")
        
        print(f"Initialized {self.encoder_type.upper()} question encoder ({model_name})")
    ...
    def forward(self, questions):
        ...
        # For BERT and RoBERTa: first token is [CLS]
        # For DistilBERT: first token is also [CLS]
        if self.encoder_type == 'distilbert':
            # DistilBERT returns only last_hidden_state
            cls_embeddings = outputs.last_hidden_state[:, 0, :]
        else:
            # BERT and RoBERTa return multiple outputs
            cls_embeddings = outputs.last_hidden_state[:, 0, :]
...
class ImprovedKGQAModel(nn.Module):
    ...
    1. Flexible transformer-based question encoder (BERT/DistilBERT/RoBERTa)
    2. Better relation prediction
    3. Topic entity extraction
    4. Cross-attention between question and KG
    """
    def __init__(self, args, nentity, nrelation, encoder_type='roberta'):
        ...
        # Question encoder with configurable transformer
    ...
    def forward(self, questions, relation_embeddings, entity_embeddings, 
        ...
        Args:
            questions: List of question strings
            relation_embeddings: [nrelation, hidden_dim*2] for ComplEx
            entity_embeddings: [nentity, hidden_dim*2] for ComplEx
        ...
        # For ComplEx, use only real part for initial scoring
        # Split relation and entity embeddings
    ...
    def predict_answers(self, questions, relation_embeddings, entity_embeddings, 
        ...
        Args:
            questions: List of question strings
            relation_embeddings: [nrelation, hidden_dim*2]
            entity_embeddings: [nentity, hidden_dim*2]
