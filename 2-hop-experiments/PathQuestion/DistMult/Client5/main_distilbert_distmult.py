...
from fkgqa_pathquestion_distmult import FederatedKGQA
import qa_model_distilbert
...
def main(args):
    ...
    logging.info("=" * 70)
    logging.info("Federated KGQA Training - DistilBERT + DistMult")
    logging.info("=" * 70)
    ...
    logging.info("=" * 70)
    logging.info(f"Model: DistMult for KG, DistilBERT for QA")
    logging.info(f"Embedding dimension: {args.hidden_dim}")
    logging.info(f"Device: {args.gpu}")
    
    fkgqa = FederatedKGQA(args, all_clients_data, global_nrelation, qa_model_distilbert)
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Federated KGQA on PathQuestion with DistilBERT + DistMult')
    ...
    # Output directories
    parser.add_argument('--name', type=str, default='fkgqa_distilbert_distmult',
    ...
    parser.add_argument('--hidden_dim', type=int, default=256,
                       help='Embedding dimension (DistMult: entities=hidden_dim, relations=hidden_dim)')
    parser.add_argument('--gamma', type=float, default=12.0,
                       help='Margin for DistMult')
    ...
    parser.add_argument('--qa_batch_size', type=int, default=16,
                       help='Batch size for QA training (smaller for DistilBERT)')
