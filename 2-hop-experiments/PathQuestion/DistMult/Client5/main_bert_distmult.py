...
def main(args):
    ...
    logging.info("=" * 70)
    logging.info("Federated KGQA Training - BERT + DistMult")
    logging.info("=" * 70)
    ...
    logging.info("=" * 70)
    logging.info(f"Model: DistMult for KG, BERT for QA")
    logging.info(f"Embedding dimension: {args.hidden_dim}")
    logging.info(f"Device: {args.gpu}")
    
    fkgqa = FederatedKGQA(args, all_clients_data, global_nrelation, qa_model_bert)
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Federated KGQA on PathQuestion with BERT + DistMult')
    ...
    # Output directories
    parser.add_argument('--name', type=str, default='fkgqa_bert_distmult',
