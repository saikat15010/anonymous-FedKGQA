"""
Server-side Evaluation Script for DistilBERT-DistMult on PathQuestion
"""
...
def load_trained_models(args, all_clients_data, global_nrelation, device):
    ...
    for client_id in range(len(all_clients_data)):
        ...
        # Initialize QA model
        qa_model = qa_model_distilbert.ImprovedKGQAModel(args, client_data['nentity'], global_nrelation)
...
def main(args):
    ...
    logging.info("=" * 70)
    logging.info("DistilBERT-DistMult Server Evaluation on PathQuestion")
    logging.info("=" * 70)
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Evaluate DistilBERT-DistMult on PathQuestion')
    ...
    parser.add_argument('--output_file', type=str, default='eval_results_distilbert.json')
