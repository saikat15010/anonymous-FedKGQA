"""
Server-side Evaluation Script for BERT-DistMult on PathQuestion
"""
...
def load_trained_models(args, all_clients_data, global_nrelation, device):
    ...
    for client_id in range(len(all_clients_data)):
        ...
        # Initialize QA model
        qa_model = qa_model_bert.ImprovedKGQAModel(args, client_data['nentity'], global_nrelation)
...
def main(args):
    ...
    logging.info("=" * 70)
    logging.info("BERT-DistMult Server Evaluation on PathQuestion")
    logging.info("=" * 70)
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Evaluate BERT-DistMult on PathQuestion')
    ...
    parser.add_argument('--output_file', type=str, default='eval_results_bert.json')
