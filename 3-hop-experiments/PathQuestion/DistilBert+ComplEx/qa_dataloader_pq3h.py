"""
QA Dataloader for PQ-3H (3-hop PathQuestion)
Same as 2-hop version — the dataloader doesn't need structural changes
since the QA format is identical (question\tanswer\tpath).
"""
            ...
            for line in f:
                ...
                if len(parts) >= 2:
                    ...
                    self.qa_pairs.append({
                        'question': question,
                        'answers': [answer],
                        'hop_count': 2  # compatibility with metrics dict
                    })
...
def load_all_pathquestion_clients(base_path, num_clients=3):
    ...
    for i in range(num_clients):
        data = load_pathquestion_client(os.path.join(base_path, f'client_{i}'))
        data['client_id'] = i
