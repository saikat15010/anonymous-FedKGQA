...
from fkgqa_dp import FederatedKGQA_DP
...
def main(args):
    """Main function to run Federated KGQA training with DP"""
    ...
    logging.info("=" * 70)
    logging.info("Federated KGQA Training - ComplEx + RoBERTa + DP")
    logging.info("=" * 70)
    ...
    logging.info("\n" + "=" * 70)
    logging.info("Initializing Federated KGQA System with DP")
    logging.info("=" * 70)
    ...
    if args.dp_enabled:
        logging.info(f"DP Enabled: sigma={args.dp_sigma}, clipping_bound={args.dp_clipping_bound}")
    else:
        logging.info("DP Disabled (baseline)")
    
    fkgqa = FederatedKGQA_DP(args, all_clients_data, global_nrelation)
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Federated KGQA with Differential Privacy')
    
    # Data parameters
    parser.add_argument('--data_path', type=str, required=True,
                       help='Path to federated dataset')
    parser.add_argument('--num_clients', type=int, default=3,
                       help='Number of clients')
    
    # Output directories
    parser.add_argument('--name', type=str, default='fkgqa_dp',
    ...
    parser.add_argument('--kg_max_rounds', type=int, default=5,
    ...
    parser.add_argument('--qa_max_rounds', type=int, default=2,
    ...
    parser.add_argument('--num_neg_qa', type=int, default=128,
                       help='Number of negative samples for QA')
    
    # Differential Privacy parameters
    parser.add_argument('--dp_enabled', action='store_true',
                       help='Enable differential privacy')
    parser.add_argument('--dp_sigma', type=float, default=0.1,
                       help='DP noise scale parameter (sigma)')
    parser.add_argument('--dp_clipping_bound', type=float, default=5.0,
                       help='DP gradient clipping bound')
    ...
    parser.add_argument('--check_per_round', type=int, default=1,
