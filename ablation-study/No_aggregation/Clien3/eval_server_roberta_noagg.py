"""
Server-side Evaluation Script for RoBERTa-ComplEx on PathQuestion
ABLATION STUDY: NO RELATION AGGREGATION

Each client has separate relation embeddings (not aggregated)
"""
...
def load_trained_models(args, all_clients_data, global_nrelation, device):
    ...
    NO GLOBAL RELATION EMBEDDINGS - each client has separate ones
    """
    ...
    logging.info("ABLATION: Loading separate relation embeddings per client")
    ...
    for client_id in range(len(all_clients_data)):
        ...
        # Handle both old format (dict) and new format (Parameter/tensor)
        if isinstance(entity_embedding, dict):
            entity_embedding = list(entity_embedding.values())[0]
        
        # Ensure it's on the correct device and is a tensor
        if isinstance(entity_embedding, nn.Parameter):
            entity_embedding = entity_embedding.data
        entity_embedding = entity_embedding.to(device)
        
        # Load relation embeddings (CLIENT-SPECIFIC - NOT GLOBAL)
        relation_embedding = torch.load(
            os.path.join(client_save_dir, 'relation_embeddings.pt'),
            map_location=device
        )
        
        # Handle both old format (dict) and new format (Parameter/tensor)
        if isinstance(relation_embedding, dict):
            relation_embedding = list(relation_embedding.values())[0]
        
        # Ensure it's on the correct device and is a tensor
        if isinstance(relation_embedding, nn.Parameter):
            relation_embedding = relation_embedding.data
        relation_embedding = relation_embedding.to(device)
        ...
        client_embeddings.append({
            ...
            'relation': relation_embedding  # Client-specific
        })
        
        logging.info(
            f"Loaded client {client_id} model with separate relation embeddings "
            f"(shape: {relation_embedding.shape})"
        )
    
    return client_models, client_embeddings


def evaluate_test_set(args, all_clients_data, client_models, client_embeddings, test_dataset):
    """
    Evaluate on test set with separate relation embeddings per client
    """
    ...
    results = {
        ...
        'ablation_type': 'NO_RELATION_AGGREGATION'
    }
...
def main(args):
    ...
    logging.info("=" * 70)
    logging.info("RoBERTa-ComplEx Server Evaluation on PathQuestion")
    logging.info("ABLATION STUDY: NO RELATION AGGREGATION")
    logging.info("=" * 70)
    ...
    client_models, client_embeddings = load_trained_models(
        args, all_clients_data, global_nrelation, device
    )
    
    # Evaluate
    logging.info("\n" + "=" * 70)
    logging.info("Starting Evaluation - NO RELATION AGGREGATION")
    logging.info("=" * 70)
    ...
    # Print results
    logging.info("\n" + "=" * 70)
    logging.info("EVALUATION RESULTS - NO RELATION AGGREGATION")
    logging.info("=" * 70)
    ...
    logging.info(f"Ablation type: NO RELATION AGGREGATION")


if __name__ == '__main__':
    parser = argparse.ArgumentParser(
        description='Evaluate RoBERTa-ComplEx on PathQuestion - NO RELATION AGGREGATION'
    )
    ...
    parser.add_argument('--output_file', type=str, default='eval_results_roberta_noagg.json')
